...
def unpack(cont, fout, quiet=False):
    ...
    if not quiet:
        print("magic good, header claims size compressed=%d, size raw=%d, cksum: 0x%0x" %
              (comp_len, raw_len, head_chk), file=sys.stderr)
        print("actual cksum: 0x%0x" % chk, file=sys.stderr)

    assert len(comp) == comp_len, "compressed length doesn't match"
    assert head_chk == chk, "compressed cksum value doesn't match"
    assert comp[:2] == LZ, "payload doesn't start with LZ header"

    dec = zlib.decompress(comp)
    assert len(dec) == raw_len, "decompressed length doesn't match"

    bytes_out = fout.write(dec)
    if not quiet:
        print("success; wrote %d bytes" % bytes_out, file=sys.stderr)

def pack(dec, fout, quiet=False):
    comp = zlib.compress(dec)
    chk = cksum(comp)
    raw_len = len(dec)
    comp_len = len(comp)

    assert comp[:2] == LZ, "compression didn't work?!"

    if not quiet:
        print("size compressed=%d, size raw=%d, cksum: 0x%08x" % (comp_len, raw_len, chk), file=sys.stderr)

    fout.write(MAGIC)
    fout.write(int.to_bytes(comp_len, length=4, byteorder=END))
    fout.write(int.to_bytes(chk, length=4, byteorder=END))
    fout.write(int.to_bytes(raw_len, length=4, byteorder=END))
    fout.write(comp)

    if not quiet:
        print("success; wrote %d bytes" % fout.tell(), file=sys.stderr)
